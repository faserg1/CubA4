...
{
	class SimpleBlockRenderModelDefinition : public virtual IBlockRenderModelDefinition
	{
	public:
		using RMaterialsMap = std::unordered_map<std::string, RMaterial>;
		explicit SimpleBlockRenderModelDefinition(const std::string &id, const BlockRenderModelData &data, RMaterialsMap materialsMap);
		~SimpleBlockRenderModelDefinition();
