...
std::shared_ptr<IAudioTrack> AudioManager::createTrack(std::shared_ptr<const IResource> resource)
{
	return createFromOgg(resource);
}

std::shared_ptr<AudioTrack> AudioManager::createFromOgg(std::shared_ptr<const IResource> resource)
{
	OggInfo ogg;
	...
	return std::make_shared<AudioTrack>(samplesBuffer, rate);
}

void AudioManager::play(std::shared_ptr<IAudioTrack> track, const CubA4::world::GlobalPosition &pos)
{
	if (!player_)
		return;
	player_->play(track, pos);
}
